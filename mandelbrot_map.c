...
typedef struct Maps
{
	int sx, ex, sy, ey;     // Local sizes
  	int gsx, gex, gsy, gey; // Global sizes
	int hs;                 // Halo size
} MAP;
...
double Mandelbrot(int i, int j, double dx, double dy, double c_real, double c_imag);
...
// Divide tasks between processors
	...
	// Tasks left
		...
		if (proc >= remainder)
		{
			*mystart = start + proc * interval + remainder; 
			*myend = *mystart + interval - 1;
		}
		else
		{
			*mystart = start + proc * interval + proc;
...
// Fill local map with values
	...
	void fillField(double *u, MAP *map)
	{
		...
		double dx = (x_max-x_min)/map->gex;
		double dy = (y_max-y_min)/map->gey;
	...
	// each processor fill its part of the field
		for (int j = map->sy; j <= (map->ey); j++)
		{
			...
			// U(i, j) = (i + j / 10.0);
				c_real = x_min + i*dx;
				c_imag = y_min + j*dy;
				U(i, j) = Mandelbrot(i, j, dx, dy, c_real, c_imag);
			}
		}
	}
